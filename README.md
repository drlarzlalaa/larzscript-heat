...
- **The variance is exactly 2Dt in every stable run,** and total heat stays 1: those are properties the schemes get right by construction, which is why looking at the *shape* (the error and the peak) is what tells you when a run has failed.
